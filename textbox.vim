function! textbox#createmappings(keys, message) abort
    for key in a:keys
        call nvim_set_keymap('n', key, ':call textboxWindow(' . string(a:message). ')<CR>', {'silent': v:true, 'nowait': v:true, 'noremap': v:true})
    endfor
endfunction

function! textboxWindow(message) abort
